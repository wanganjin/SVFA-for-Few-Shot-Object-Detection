...
from .anlf import Attention_min, Attention_mul


@HEADS.register_module()
class SVFARoIHead(StandardRoIHead):
    """Roi head for `SVFA <https://arxiv.org/tbd>`.
    Args:
    ...
    def __init__(self,
        ...
        self.fuse_mode_min = Attention_min(feature_dim = 2048)
        self.fuse_mode_mul = Attention_mul(feature_dim = 2048)
    ...
    def _optimized_bbox_forward_train(self, query_feats: List[Tensor],
        ...
        # ASSS (Adaptive Support Sample Selection)
        supp_ids = []  # Initialize support sample index list
        num_supp_per_im = num_supp_shots + math.ceil(math.sqrt(num_supp_shots))  # Number of support samples per image

        for img_id in range(batch_size):  # Iterate through each image
            # Randomly select a label from the query image
            random_index = np.random.choice(range(query_gt_labels[img_id].size(0)))
            random_query_label = query_gt_labels[img_id][random_index].item()
            pos_supp_id = []  # Positive sample index list
            neg_supp_id = []  # Negative sample index list

            # Calculate similarity between support features and query features
            query_feat_pool = query_feats[0][img_id].mean(dim=[1, 2])  # Convert to vector through global average pooling
            similarities = []
            for i in range(support_feats[0].size(0)):
                # Calculate similarity, e.g., cosine similarity
                support_feat_pool = support_feats[0][i].mean(dim=[1, 2])  # Also convert to vector through global average pooling
                similarity = torch.nn.functional.cosine_similarity(query_feat_pool.unsqueeze(0), support_feat_pool.unsqueeze(0), dim=1)
                similarities.append((i, similarity.item()))
                
            # Select negative samples based on labels and similarity
            neg_similarities = [(idx, sim) for idx, sim in similarities if support_gt_labels_[idx].item() != random_query_label]
                
            for i in range(support_feats[0].size(0)):
                if support_gt_labels[i] == random_query_label:
                    pos_supp_id.append(i)
            while len(pos_supp_id) + len(neg_supp_id) < num_supp_per_im:
                if neg_similarities:
                    neg_similarities.sort(key=lambda x: x[1])
                    neg_supp_id = [idx for idx, _ in neg_similarities[:num_supp_per_im - len(pos_supp_id)]]

            # Merge positive and negative sample indices
            supp_id = pos_supp_id + neg_supp_id
        ...
        for k in range(num_supp_per_im):
            ...
            # prototype = self.enhancement_mode2(prototype)
            # ANLF
            
            agg1 = self.linear1(self.fuse_mode_mul(query_roi_feats, prototype))
            agg2 = self.linear2(self.fuse_mode_min(query_roi_feats, prototype))
            agg3 = self.linear4(
                # self.fuse_mode_add(query_roi_feats, prototype)
                torch.cat([query_roi_feats,prototype], dim=-1)
                # torch.cat([query_roi_feats, self.enhancement_mode2(prototype)], dim=-1)
            )
            agg = self.linear3(
    ...
    def simple_test_bboxes(
        ...
        # rpt_support_feat = self.enhancement_mode2(rpt_support_feat)

        # ANLF
        
        agg1 = self.linear1(self.fuse_mode_mul(query_roi_feats, rpt_support_feat))
        agg2 = self.linear2(self.fuse_mode_min(query_roi_feats, rpt_support_feat))
        agg3 = self.linear4(
            # self.fuse_mode_add(query_roi_feats, rpt_support_feat)
            torch.cat([query_roi_feats, rpt_support_feat], dim=-1)
            # torch.cat([query_roi_feats, self.enhancement_mode2(rpt_support_feat)], dim=-1)
        )
        agg = self.linear3(
            torch.cat([agg1, agg2, agg3, query_roi_feats], dim=-1)
        )
