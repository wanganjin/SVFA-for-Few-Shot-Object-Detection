...
try:
    from mmcv.runner import is_module_wrapper
except ImportError:
    try:
...
def parse_args():
    ...
    parser.add_argument(
        '--save-img-dir', 
        help='directory where both original images and detection results will be saved')
    parser.add_argument(
        '--show-score-thr',
        type=float,
        default=0.3,
...
def main():
    ...
    assert args.out or args.eval or args.show \
        or args.show_dir or args.save_img_dir, (
            'Please specify at least one operation (save/eval/show the '
            'results / save the results) with the argument "--out", "--eval"',
            '"--show", "--show-dir" or "--save-img-dir"')
    ...
    if not os.path.isabs(args.config):
        ...
        print(f"Using absolute config path: {abs_config}")
        args.config = abs_config
    
    # Set environment variable to pass config path to ffa.py
    os.environ['SVFA_CONFIG_PATH'] = args.config
    print(f"Set config path to environment variable: {args.config}")
    ...
    if args.save_img_dir:
        # Create directories for saving original images and detection results
        mmcv.mkdir_or_exist(args.save_img_dir)
        mmcv.mkdir_or_exist(os.path.join(args.save_img_dir, 'original'))
        mmcv.mkdir_or_exist(os.path.join(args.save_img_dir, 'detection'))
    ...
    if not distributed:
        ...
        # If save_img_dir is specified, modify single_gpu_test function behavior
        if args.save_img_dir:
            # Use custom test function to save original images and detection results
            from mmcv.image import tensor2imgs
            
            def custom_single_gpu_test(model, data_loader, show=False, out_dir=None, show_score_thr=0.3, save_img_dir=None):
                model.eval()
                results = []
                ...
                for i, data in enumerate(data_loader):
                    ...
                    if show or out_dir or save_img_dir:
                        ...
                        for j, (img, img_meta) in enumerate(zip(imgs, img_metas)):
                            ...
                            if save_img_dir:
                                ...
                                original_file = os.path.join(save_img_dir, 'original', ori_filename)
                                mmcv.imwrite(img_show, original_file)
                                detect_file = os.path.join(save_img_dir, 'detection', ori_filename)
                                model_module = model.module if is_module_wrapper(model) else model
                                model_module.show_result(
                                    img_show,
                                    result[j],
                                    show=False,
                                    out_file=detect_file,
                                    score_thr=show_score_thr)
                            if out_dir:
                                out_file = os.path.join(out_dir, ori_filename)
                            else:
                                out_file = None
                            if show or out_file:
                                model_module = model.module if is_module_wrapper(model) else model
            ...
            outputs = custom_single_gpu_test(
                model, data_loader, args.show, args.show_dir, show_kwargs['show_score_thr'], args.save_img_dir)
        else:
            # Use original single_gpu_test function
        ...
        # Distributed testing currently doesn't support saving original images and detection results
        if args.save_img_dir:
            print("Warning: Distributed testing mode doesn't support saving original images and detection results simultaneously, ignoring --save-img-dir parameter")
